...
class GatedCrossLingualFusion(nn.Module):
    def __init__(self, d: int):
        super().__init__()
        self.W_F = nn.Linear(2*d, d)
        self.b_F = nn.Parameter(torch.zeros(d))
        self.W_G = nn.Linear(2*d, 1)
        self.b_G = nn.Parameter(torch.zeros(1))

    def forward(self, audio_norm: torch.Tensor, text_norm: torch.Tensor) -> torch.Tensor:
        batch, seq_len, d = audio_norm.shape
        text_global = text_norm.mean(dim=1)
        text_global_expanded = text_global.unsqueeze(1).expand(-1, seq_len, -1)
        # Rsrc, Rtgt: (batch, d)
        concat = torch.cat([audio_norm, text_global_expanded], dim=-1)        # (batch, 2d)
        X = torch.tanh(self.W_F(concat) + self.b_F)     # (batch, d)
        G = torch.sigmoid(self.W_G(concat) + self.b_G)  # (batch, 1)
        F = G * X + (1 - G) * text_global_expanded
        return F
...
class MemoCMT(nn.Module):
    def __init__(
        ...
        self.fusion = GatedCrossLingualFusion(cfg.fusion_dim)
    ...
    def forward(
        ...
        fusion_norm = self.fusion(audio_norm, text_norm)
